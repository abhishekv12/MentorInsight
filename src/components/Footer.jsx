...
const Footer = () => {
  ...
  const Modal = ({ title, children }) => (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.7)", zIndex:9999,
      ...
      <div style={{ background:"#0d1520", border:"1px solid rgba(255,255,255,.1)", borderRadius:16,
        ...
        <div style={{ padding:"18px 20px", borderBottom:"1px solid rgba(255,255,255,.07)",
          ...
          <button onClick={() => setModal(null)} style={{ background:"none", border:"none",
            fontSize:22, cursor:"pointer", color:"#475569", lineHeight:1, padding:"0 4px" }}>×</button>
        </div>
        <div style={{ padding:"20px", fontSize:14, color:"#64748b", lineHeight:1.8 }}>
          {children}
        </div>
      </div>
    </div>
  ...
  return (
    <>
      <style>{`
        ...
        .lp-footer-desc { font-size: 13px; color: #334155; line-height: 1.8; margin: 0 0 20px; }
        .lp-footer-badge {
        ...
        .lp-footer-badge-dot { width: 5px; height: 5px; border-radius: 50%; background: #10b981; flex-shrink: 0; }
        .lp-footer-col-h4 {
          font-size: 11px;
          font-weight: 700;
          color: #334155;
        ...
        .lp-footer-divider { max-width: 1240px; margin: 0 auto 24px; height: 1px; background: rgba(255,255,255,.05); }
        .lp-footer-bottom {
        ...
        @media(max-width: 560px) {
          .lp-footer { padding: 44px 5vw 24px; }
          .lp-footer-grid {
            grid-template-columns: 1fr;
            gap: 32px;
            margin-bottom: 36px;
          }
          /* Show brand col first, then cols in order */
          .lp-footer-bottom {
            flex-direction: column;
            align-items: flex-start;
            gap: 10px;
          }
          .lp-footer-legal { flex-wrap: wrap; gap: 14px; }
          .lp-footer-secure { display: none; }
        }
      `}</style>
      ...
      <footer className="lp-footer">
        ...
        <div className="lp-footer-bottom">
          <p className="lp-footer-copy">© {year} <span>MentorInsight</span> — All rights reserved.</p>
      ...
      {modal === "privacy" && (
        <Modal title="Privacy Policy">
          <p style={{ color: "#94a3b8" }}><strong style={{ color: "#e2e8f0" }}>Data Collection:</strong> MentorInsight collects institution data solely for operating the ERP system.</p>
          ...
          <p style={{ color: "#94a3b8" }}><strong style={{ color: "#e2e8f0" }}>Review Anonymity:</strong> Student reviews are stored anonymously by default.</p>
      ...
      {modal === "terms" && (
        <Modal title="Terms of Service">
          <p style={{ color: "#94a3b8" }}><strong style={{ color: "#e2e8f0" }}>Usage:</strong> MentorInsight is licensed for use by registered educational institutions only.</p>
          ...
          <p style={{ color: "#94a3b8" }}><strong style={{ color: "#e2e8f0" }}>Review System:</strong> Faculty reviews are tools for institutional improvement only.</p>
          ...
          {[
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px",
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: "#64748b", wordBreak: "break-all" }}>{c.val}</div>
              </div>
            </div>
          ))}
        </Modal>
