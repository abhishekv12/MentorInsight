...
const ratingLabel = (r) => ["", "Poor", "Below Average", "Average", "Good", "Excellent"][r] || "Tap a star";

const StarInput = ({ value, onChange, hoveredStar, setHoveredStar }) => (
  <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
    {[1, 2, 3, 4, 5].map(i => {
      const active = hoveredStar ? i <= hoveredStar : i <= value;
      const color = active ? STAR_COLORS[Math.max(hoveredStar || value, 1) - 1] : "#e2e8f0";
      return (
        <i key={i}
          className={`fa-${active ? "solid" : "regular"} fa-star`}
          style={{ fontSize: "28px", color, cursor: "pointer", transition: "all 0.15s ease", transform: active ? "scale(1.1)" : "scale(1)" }}
    ...
    <span style={{ marginLeft: "10px", color: "#6366f1", fontSize: "13px", fontWeight: "700", minWidth: "100px" }}>
...
const FacultyReviewStudent = ({ student }) => {
  ...
  const fetchCampaigns = async () => {
    ...
    try {
      const [campRes, subRes] = await Promise.allSettled([
        axios.get(`http://localhost:5000/api/student/review-campaigns/${batchId}`),
        axios.get(`http://localhost:5000/api/student/my-reviews/${student?.rollNo}`),
      ]);
      if (campRes.status === "fulfilled" && campRes.value.data.campaigns?.length) {
        setCampaigns(campRes.value.data.campaigns.filter(c => c.status === "active"));
      } else {
  ...
  const handleSubmit = async () => {
    if (overallRating === 0) { alert("Please give an overall rating."); return; }
    if (selected.questions?.some((_, i) => !ratings[i])) {
      alert("Please rate all questions."); return;
    }
    setSubmitting(true);
    try {
    ...
    setSubmitted(prev => new Set([...prev, selected._id]));
    setSelected(null);
    setSubmitting(false);
  };
    ...
    <div className="frs-root">
      ...
      <div className="frs-header">
        <div>
          ...
          <p className="frs-subtitle">Help your institution improve by sharing honest feedback about your teachers</p>
        </div>
      ...
      {campaigns.length === 0 && (
        <div className="frs-empty">
          <div className="frs-empty-icon"><i className="fa-solid fa-star-half-stroke"></i></div>
          <h3>No Active Reviews</h3>
          <p>Your administrator will post review campaigns here when they need your feedback on teachers.</p>
        </div>
      )}
      ...
      {activeCampaigns.length > 0 && (
        <div className="frs-section">
          <div className="frs-section-header">
            <i className="fa-solid fa-circle" style={{ color: "#22c55e", fontSize: "10px" }}></i>
            Pending Your Review
          ...
          <div className="frs-campaigns-grid">
            {activeCampaigns.map(c => {
              ...
              return (
                <div key={c._id} className={`frs-camp-card ${urgent ? "frs-urgent" : ""}`}>
                  {urgent && (
                    <div className="frs-deadline-flag">
                      <i className="fa-solid fa-clock"></i> {dl <= 0 ? "Closes today!" : `${dl}d left`}
                    </div>
                  )}
                  ...
                  <div className="frs-camp-meta-row">
                    {c.questions?.length > 0 && (
                      <span><i className="fa-solid fa-list-check"></i> {c.questions.length} questions</span>
                    )}
                    {c.deadline && (
                      <span style={{ color: urgent ? "#ef4444" : "inherit" }}>
                        <i className="fa-solid fa-calendar-xmark"></i> Due {new Date(c.deadline).toLocaleDateString()}
                      </span>
                    )}
                  </div>
                  <button className="frs-review-btn" onClick={() => openCampaign(c)}>
      ...
      {completedCampaigns.length > 0 && (
        <div className="frs-section">
          <div className="frs-section-header">
            ...
            Submitted
            <span className="frs-section-count">{completedCampaigns.length}</span>
          </div>
          <div className="frs-completed-list">
            {completedCampaigns.map(c => (
              <div key={c._id} className="frs-done-card">
                <div className="frs-done-icon"><i className="fa-solid fa-circle-check"></i></div>
                <div>
                  <div className="frs-done-title">{c.title}</div>
                  <div className="frs-done-meta">{c.teacherName} · {c.batchName}</div>
                </div>
      ...
      <div className="frs-notice">
        ...
        <div>
          ...
          <p>Responses are anonymized by default. The admin sees aggregated ratings, not individual names.</p>
        </div>
      </div>

      {/* ══ REVIEW MODAL ════════════════════════════════════ */}
      {selected && (
        <div className="frs-modal-overlay" onClick={() => !submitting && setSelected(null)}>
          <div className="frs-modal" onClick={e => e.stopPropagation()}>
            <div className="frs-modal-header">
              <div>
                ...
                <p className="frs-modal-teacher">
                  <i className="fa-solid fa-user-tie"></i> {selected.teacherName}
                </p>
              </div>
              <button className="frs-modal-close" onClick={() => setSelected(null)} disabled={submitting}>
            ...
            <div className="frs-modal-body">
              ...
              <div className="frs-q-block frs-overall-block">
                ...
                <StarInput value={overallRating} onChange={setOverallRating}
                  hoveredStar={hoveredOverall} setHoveredStar={setHoveredOverall} />
              </div>
              ...
              {selected.questions?.map((q, i) => (
                <div key={i} className="frs-q-block">
                  ...
                  <StarInput value={ratings[i] || 0}
                    onChange={v => setRatings(prev => ({ ...prev, [i]: v }))}
                    hoveredStar={hoveredStars[i] || 0}
                    setHoveredStar={v => setHoveredStars(prev => ({ ...prev, [i]: v }))} />
                </div>
              ))}
              ...
              <div className="frs-q-block">
                <div className="frs-q-label">
                  <span className="frs-q-num"><i className="fa-solid fa-comment"></i></span>
              ...
              <label className="frs-anon-toggle">
                <div className={`frs-toggle-switch ${anonymous ? "on" : "off"}`}
                ...
                <div>
                  ...
                  <span className="frs-anon-sub">
                    {anonymous ? "Your name won't be shared with admin" : "Your name will be visible to admin"}
                  </span>
                </div>
              </label>
            </div>
            ...
            <div className="frs-modal-footer">
              <button className="frs-cancel-btn" onClick={() => setSelected(null)} disabled={submitting}>
                Cancel
              </button>
              <button className="frs-submit-btn" onClick={handleSubmit} disabled={submitting}>
                {submitting
